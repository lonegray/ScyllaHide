...
#define MAX_NATIVE_HOOKS 20
    ...
    //typedef BOOL(WINAPI *t_DllMain)(HINSTANCE hInstDLL, DWORD dwReason, LPVOID lpvReserved);
    typedef void (WINAPI *t_GetSystemTime)(LPSYSTEMTIME lpSystemTime); // kernel32.dll/kernelbase.dll
    typedef void (WINAPI *t_GetLocalTime)(LPSYSTEMTIME lpSystemTime); // kernel32.dll/kernelbase.dll
    typedef DWORD(WINAPI *t_timeGetTime)(void); // winmm.dll (sometimes GetTickCount)
    typedef DWORD(WINAPI *t_GetTickCount)(void); // kernel32.dll/kernelbase.dll
